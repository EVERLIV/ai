#!/usr/bin/env bash
#
# Установка бэкенда ИИ-чата. Ставит Node-сервис, службу systemd и проверяет запуск.
#
# ЗАПУСК (ключ Anthropic передаётся аргументом или спрашивается интерактивно):
#   bash install-chat.sh
#
# В скрипте НЕТ заполнителей вида <...> — ничего подставлять не нужно.

set -euo pipefail

APP_DIR=/opt/arendacity-chat
ENV_FILE=/etc/arendacity-chat.env
SERVICE=/etc/systemd/system/arendacity-chat.service
PORT=8787

echo "=== 1/6 Проверяем Node.js"
if ! command -v node >/dev/null 2>&1; then
  echo "Node.js не найден. Устанавливаю Node 20..."
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "Нужен Node 18+ (в нём есть встроенный fetch). Обновите Node." >&2
  exit 1
fi

echo "=== 2/6 Спрашиваем ключ Anthropic"
key="${1:-}"
if [ -z "$key" ]; then
...
echo "=== 3/6 Записываем сервис"
mkdir -p "$APP_DIR"
cat > "$APP_DIR/chat-server.mjs" <<'EOF_SERVER'
/**
...
echo "  файл: $APP_DIR/chat-server.mjs ($(wc -c < "$APP_DIR/chat-server.mjs") байт)"

echo "=== 4/6 Настройки окружения"
cat > "$ENV_FILE" <<ENVEOF
ANTHROPIC_API_KEY=$key
SUPABASE_URL=https://api.arendacity.com
SUPABASE_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb2xlIjoiYW5vbiIsImlzcyI6InN1cGFiYXNlIiwiaWF0IjoxNzc4ODQyOTQwLCJleHAiOjE5MzY1MjI5NDB9.uK1BksB1rl0vNAlUc2nVpkqECeiWD9CKx0rIfHUlyWA
PORT=$PORT
ENVEOF
chmod 600 "$ENV_FILE"
echo "  файл: $ENV_FILE (права 600)"

NODE_BIN="$(command -v node)"
echo "=== 5/6 Служба systemd"
cat > "$SERVICE" <<SERVICEEOF
[Unit]
Description=AI chat backend for arendacity.com
After=network.target

[Service]
Type=simple
WorkingDirectory=$APP_DIR
ExecStart=$NODE_BIN $APP_DIR/chat-server.mjs
Restart=always
RestartSec=5
...
systemctl enable arendacity-chat >/dev/null 2>&1 || true
systemctl restart arendacity-chat
sleep 3

echo "=== 6/6 Проверка"
if ! systemctl is-active --quiet arendacity-chat; then
  echo "СЛУЖБА НЕ ЗАПУСТИЛАСЬ. Логи:" >&2
  journalctl -u arendacity-chat -n 30 --no-pager >&2
  exit 1
fi
echo "  служба активна"

health="$(curl -s --max-time 10 "http://127.0.0.1:$PORT/health" || true)"
echo "  /health: ${health:-нет ответа}"

echo "  пробный запрос к ИИ..."
reply="$(curl -s --max-time 60 -X POST "http://127.0.0.1:$PORT/api/chat"   -H 'Content-Type: application/json'   -d '{"messages":[{"role":"user","content":"Привет! Сколько объектов в аренду?"}]}' | head -c 400 || true)"
echo "  ответ: ${reply:-пусто}"

case "$reply" in
  *'"text"'*)
    echo
    echo "ГОТОВО: бэкенд работает на 127.0.0.1:$PORT"
    echo
    echo "Осталось открыть его наружу. Добавьте в nginx (сервер api.arendacity.com):"
    echo
  ...
  *)
    echo
    echo "Бэкенд запущен, но ИИ не ответил. Смотрите логи:" >&2
