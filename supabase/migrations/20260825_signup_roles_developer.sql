-- =============================================================================
-- Signup roles: seeker | owner | realtor | agency | developer
-- Run on self-hosted after self_hosted_agencies.sql + self_hosted_developers.sql
-- =============================================================================
...
BEGIN
  ...
  v_account_type := CASE v_meta_type
    ...
    WHEN 'developer' THEN 'developer'::public.profile_account_type
  ...
  v_dev_name := COALESCE(
    NULLIF(trim(NEW.raw_user_meta_data->>'developer_name'), ''),
    NULLIF(trim(v_agency_name), ''),
    'Застройщик'
  );
  v_dev_subtype := lower(trim(COALESCE(
    NEW.raw_user_meta_data->>'developer_subtype',
    'apartment_developer'
  )));
  IF v_dev_subtype NOT IN ('apartment_developer', 'frame_house_builder') THEN
    v_dev_subtype := 'apartment_developer';
  END IF;
  ...
  VALUES (
    ...
    v_account_type,
    CASE
      WHEN v_account_type = 'agency'::public.profile_account_type THEN v_agency_name
      WHEN v_account_type = 'developer'::public.profile_account_type THEN v_dev_name
      ELSE ''
    END,
  ...
  IF v_account_type = 'developer'::public.profile_account_type THEN
    ...
    VALUES (
      v_dev_name,
      COALESCE(NEW.raw_user_meta_data->>'developer_about', ''),
      v_dev_subtype::public.developer_subtype,
      COALESCE(NEW.raw_user_meta_data->>'developer_city', ''),
      COALESCE(NEW.raw_user_meta_data->>'developer_region', 'Иркутская область')
    )
    RETURNING id INTO v_developer_id;

    INSERT INTO public.developer_members (developer_id, user_id, role)
    VALUES (v_developer_id, NEW.id, 'owner');
  END IF;
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Signup: seeker|owner|realtor|agency|developer; agency/developer create org+owner membership';
