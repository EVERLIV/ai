...
function featureIcon(label: string) {
  ...
  if (
    ...
    value.includes("интернет")
  )
    return WifiHigh;
  if (value.includes("конди") || value.includes("вентил")) return Snowflake;
  if (value.includes("охран") || value.includes("видеонаблю"))
    return ShieldCheck;
...
export default function AIPropertyWizard({
  ...
  const TYPE_ORDER = isResidential
    ? RESIDENTIAL_TYPE_ORDER
    : COMMERCIAL_TYPE_ORDER;
  ...
  const districts = useMemo(
    () => [
      "Любой",
      ...uniqueNonEmpty(properties.map((property) => property.district)),
    ],
    [properties],
  );
  ...
  const classOptions = useMemo(() => {
    ...
    return [
      "Любой",
      ...uniqueNonEmpty(source.map((property) => property.class)).filter(
        (value) => value !== "-",
      ),
    ];
  }, [pool, properties]);
  ...
  const shortlist = useMemo(() => {
    return properties.filter((property) => {
      ...
      if (selectedFeatures.length > 0) {
        const values = (property.features || []).map((feature) =>
        ...
        if (
          !selected.every((feature) =>
            values.some(
              (value) => value.includes(feature) || feature.includes(value),
            ),
          )
        ) {
          return false;
  ...
  const buildFallbackResult = (): AIResponse => {
    ...
    const picks = source
      .map((property) => {
        ...
        const propertyFeatures = (property.features || []).map((feature) =>
          feature.toLowerCase().trim(),
        );
        ...
        return {
          ...
          reason:
            [
              ...
              district !== "Любой"
                ? `локация ${property.district || "без района"}`
                : null,
              budgetMin || budgetMax
              ...
              .filter(Boolean)
              .slice(0, 2)
              .join(", ") || "Хорошо совпадает с базовыми параметрами запроса.",
  ...
  const runAI = async () => {
    ...
    try {
      const source = (shortlist.length > 0 ? shortlist : properties).slice(
        0,
        60,
      );
      const liteList = source.map((property) => ({
        ...
        ceiling_height: property.ceiling_height
          ? Number(property.ceiling_height)
          : null,
      }));
          ...
          {loading ? (
            <div className="py-6 flex flex-col items-center gap-2">
              ...
              <div className="text-[11px] text-muted-foreground">
                ИИ анализирует {shortlist.length || properties.length}{" "}
                объектов...
              </div>
            </div>
            ...
            <div className="py-3 text-center">
              ...
              <div className="text-[10px] text-muted-foreground mb-2">
                {result?.summary || "Попробуйте смягчить параметры"}
              </div>
              <button
                onClick={reset}
            ...
            <>
              <div className="bg-muted/40 p-2.5">
                <div className="flex items-start gap-1.5">
                  ...
                  <div className="text-[10.5px] text-foreground/90 leading-relaxed">
                    {result.summary}
                  </div>
                </div>
              </div>
                      ...
                      {pick.highlights?.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {pick.highlights.map((highlight, index) => (
                            <span
                              key={index}
                              className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] bg-foreground/[0.06] text-primary border border-primary/20 font-medium"
                            >
      ...
      <div className="px-3 pb-3 min-h-[120px] min-w-0">
        ...
        {step === 2 && (
          <div className="animate-fade-in-up">
            ...
            <p className="mt-2 text-[10px] text-muted-foreground">
              Если сценарий нестандартный, просто опишите его на последнем шаге.
            </p>
          </div>
        )}
          ...
          <div className="animate-fade-in-up space-y-2.5">
            ...
            <div>
              <p className="text-[11px] text-foreground/80 mb-1.5">
                {isLandFlow ? "Какой участок нужен" : "Состояние"}
              </p>
              <div className="flex flex-wrap gap-1">
                {(isLandFlow
                  ? LAND_CONDITIONS
        ...
        {step === 7 && (
          <div className="animate-fade-in-up space-y-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[11px] text-foreground/80 mb-1.5">
                {isLandFlow ? "Комментарий" : "Опишите задачу своими словами"}
              </p>
              <textarea
                value={notes}
                ...
                placeholder={
                  isLandFlow
                    ? "Например: первая линия, подъезд для фур, рядом электричество и вода..."
                    : "Например: нужен офис с парковкой, входной группой и хорошим ремонтом..."
                }
                rows={3}
        ...
        <div className="sticky bottom-0 -mx-3 mt-3 border-t border-border bg-card/95 px-3 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur">
          ...
          <div className="mt-2 flex items-center justify-between gap-3">
            ...
            {step < STEPS.length - 1 ? (
              ...
              <button
                onClick={runAI}
                className="inline-flex items-center gap-1 rounded-md px-3 py-1.5 text-[11px] font-medium bg-primary text-primary-foreground hover:opacity-90 transition-opacity"
              >
