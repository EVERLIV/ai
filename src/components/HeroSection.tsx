  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center pt-16">
      ...
      <div className="relative z-10 container mx-auto px-4 lg:px-8 py-20">
        <div className={`max-w-3xl mx-auto text-center ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}>
          ...
          <div className="max-w-2xl mx-auto" style={{ position: "relative", zIndex: 100 }}>
            <div className="relative">
              <div ref={searchBarRef} className="flex bg-card shadow-[0_4px_24px_-4px_rgba(0,0,0,0.12)] border border-border">
                {/* Type select */}
                <div className="relative border-r border-border shrink-0">
                  <select
                    value={searchType}
                    onChange={(e) => setSearchType(e.target.value)}
                    className="appearance-none w-24 sm:w-36 px-3 sm:px-4 py-3 sm:py-3.5 bg-transparent text-xs sm:text-sm font-medium text-foreground focus:outline-none cursor-pointer pr-7"
                  >
                ...
                <input
                  ...
                  className="flex-1 px-3 sm:px-4 py-3 sm:py-3.5 bg-transparent text-xs sm:text-sm text-foreground placeholder:text-muted-foreground focus:outline-none min-w-0"
                />

                {/* Button */}
                <button
                  onClick={handleSearch}
                  className="flex items-center justify-center gap-2 px-4 sm:px-7 py-3 sm:py-3.5 bg-primary text-primary-foreground text-xs sm:text-sm font-semibold hover:opacity-90 transition-opacity shrink-0"
                >
        ...
        {/* Stats */}
        <div className={`flex justify-center gap-6 sm:gap-20 mt-10 sm:mt-16 ${isVisible ? "animate-fade-in-up" : "opacity-0"}`} style={{ animationDelay: "0.3s", position: "relative", zIndex: 0 }}>
          {stats.map((s, i) => (
            <div key={s.label} className="text-center">
              <div className="font-display text-xl sm:text-4xl font-bold text-foreground whitespace-nowrap">
                {counts[i].toLocaleString("ru-RU")}{s.suffix}
              </div>
              <div className="text-[10px] sm:text-xs text-muted-foreground mt-0.5 sm:mt-1 uppercase tracking-wide whitespace-nowrap">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
