...
function loadVisitor(): Visitor | null {
  try {
    ...
    if (v?.name?.trim().length >= 2)
      return { name: v.name.trim(), phone: v.phone?.trim() || "" };
  } catch {
    /* ignore */
  }
  return null;
...
export default function PropertyAIChat({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setThinking(false);
        const hello = visitor.name
          ? `${visitor.name}, здравствуйте!`
          : "Здравствуйте!";
        setMsgs([
          {
            role: "assistant",
            ...
            content: propertyAddress
              ? `${hello} Я Анастасия, консультант ДАДАТУТ.\nПомогу с вопросами по объекту «${propertyAddress}».\n\nЧто подсказать?`
              : `${hello} Я Анастасия, консультант ДАДАТУТ.\nПомогу подобрать помещение — офис, склад, торговое или жильё.\n\nЧто ищете?`,
          },
        ]);
      }, 1100);
    }, delay);
  ...
  const _replyAfterPause = useCallback((text: string, pauseMs = 1200) => {
    setThinking(true);
    setTimeout(() => {
      setThinking(false);
      setMsgs((p) => {
        ...
        const updated = p.map((m) =>
          m.role === "user" ? { ...m, status: "read" as Status } : m,
        );
        return [
          ...updated,
          {
            role: "assistant",
            content: stripVoiceStageDirections(text),
            time: ts(),
          },
        ];
      });
  ...
  const sendAI = async (history: Msg[], name: string) => {
    ...
    try {
      ...
      const msg =
        e instanceof Error && e.message && e.message !== "bad response"
          ? e.message
          : "Ошибка сети";
  ...
  const showStarters =
    !loading && !thinking && msgs.filter((m) => m.role === "user").length <= 1;
      ...
      >
        ...
        >
          <div className="relative w-12 h-12 bg-card border border-border shadow-lg flex items-center justify-center">
            ...
            <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-primary text-primary-foreground text-[9px] font-bold flex items-center justify-center">
              1
            </span>
          </div>
          ...
          {visitor &&
            (isVoiceMode ? (
              ...
              <button
                ...
                onClick={() =>
                  void startVoiceCall({ propertyId, propertyAddress })
                }
        ...
        {!visitor ? (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
              Чтобы консультант мог обратиться к вам по имени. Телефон — по
              желанию.
            </p>
              ...
              {transcripts.length === 0 && (
                <div className="text-center text-muted-foreground text-sm py-10 px-4">
                  <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-emerald-500/10 flex items-center justify-center">
                    <Mic
                      className={`w-7 h-7 text-emerald-600 ${isSpeaking ? "" : "animate-pulse"}`}
                    />
                  </div>
          ...
          <>
            ...
            >
              <div className="flex flex-col gap-1">
                {msgs.map((m, i) => {
                  ...
                  const _lastInGroup = !next || next.role !== m.role;
                  ...
                  return (
                    <div
                      key={i}
                      className={`flex ${isUser ? "justify-end" : "justify-start"} ${mt}`}
                    >
                      <div
                        className={`max-w-[78%] px-3.5 pt-2 pb-1.5 ${radius} ${bg}`}
                      >
                        {/* Контент */}
                        ...
                        <div className="flex items-center justify-end gap-1 mt-1">
                          <span className="text-[10px] leading-none text-muted-foreground/50 select-none">
                            {m.time}
                          </span>
                          {isUser &&
                            (m.status === "read" ? (
            ...
            {showStarters && (
              <div
                className="shrink-0 px-3 py-2 bg-card border-t border-border/40 flex gap-1.5 overflow-x-auto"
            ...
            >
              ...
              <input
                ...
                maxLength={1000}
                placeholder={"Спросите про объекты…"}
