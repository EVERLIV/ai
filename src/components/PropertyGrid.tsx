...
export default function PropertyGrid({ filters }: { filters?: PropertyFilters }) {
  ...
  return (
    <section ref={ref} id="property-results" className="py-8 lg:py-16 scroll-mt-20">
      <div className={`container mx-auto px-4 lg:px-8 ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}>
        <div className="flex items-end justify-between mb-4 lg:mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm lg:text-base hidden lg:block">Лучшие предложения на рынке</p>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="hidden lg:grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.slice(0, 12).map((p) => (
                <PropertyGridCard key={p.id} property={p} onOpenPKK={setActivePKK} />
              ))}
            </div>
          </>
        )}
