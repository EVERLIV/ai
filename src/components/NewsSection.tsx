...
const news = sortNewsByDate(FALLBACK_NEWS_POSTS)
  .slice(0, 5)
  .map((post) => ({
    id: post.id,
    tag: post.category,
        ...
        <div className="flex items-end justify-between mb-8 flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Актуальная подборка по офисам, складам и торговой недвижимости
            </p>
          </div>
          <Link
            to="/news"
          ...
          >
            ...
            <div className="p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 ${tagColor[news[0].tag] ?? "bg-muted text-muted-foreground"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 ${tagDot[news[0].tag] ?? "bg-muted-foreground"}`}
                  />
                  {news[0].tag}
                </span>
          ...
          {news.slice(1).map((item) => (
            ...
            >
              ...
              <div className="flex flex-col gap-1.5 min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span
                    className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 ${tagColor[item.tag] ?? "bg-muted text-muted-foreground"}`}
                  >
                    <span
                      className={`w-1 h-1 ${tagDot[item.tag] ?? "bg-muted-foreground"}`}
                    />
                    {item.tag}
                  </span>
