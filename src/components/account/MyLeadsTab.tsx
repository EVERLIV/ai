...
function useMyLeads() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const propSelectFallback =
        "id, address, type, area, price, cover_photo, extras";
      ...
      if (!myProps.length)
        return {
          leads: [] as Lead[],
          properties: {} as Record<string, PropertyInfo>,
        };
      ...
      <div>
        ...
        <div className="space-y-3">
          {[1, 2].map((i) => (
            <div
              key={i}
              className="bg-card rounded-lg h-[120px] animate-pulse"
            />
          ))}
        </div>
      </div>
