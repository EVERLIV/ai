...
import DealTypeSheet, { dealChoiceLabel, type DealChoice } from "@/components/mobile/DealTypeSheet";
import FilterPickerField from "@/components/mobile/FilterPickerField";
import PropertyTypeOverlay, { propertyTypeAccusative } from "@/components/mobile/PropertyTypeOverlay";
...
const ROOM_FILTER_TYPES = new Set(["Квартира", "Комната", "Апартаменты", "Доля"]);
...
export default function MobileHomeSearch() {
  ...
  const showRooms = segment === "residential" && ROOM_FILTER_TYPES.has(propertyType);
  ...
  const catalogBase = buildCatalogUrl({
    ...
    deal,
    types: propertyType,
    rooms: showRooms && rooms && rooms !== "Свободная планировка" ? rooms : undefined,
  ...
  const handleMap = () => {
    ...
    else navigate(segment === "residential" ? "/zhilaya/catalog?view=map" : "/catalog?view=map");
  };
  ...
  return (
    <section className="lg:hidden bg-[#E8F4FF] dark:bg-primary/10 px-4 pt-[4.5rem] pb-4">
      <div className="rounded-xl bg-card shadow-sm p-3 space-y-2">
          ...
          {(["residential", "commercial"] as const).map((s) => (
            <button
              ...
              className={cn(
                ...
                segment === s ? "bg-card text-foreground shadow-sm" : "text-muted-foreground",
              )}
            >
        ...
        <FilterPickerField
          label="Тип объекта"
          value={segment === "residential" ? propertyTypeAccusative(propertyType) : propertyType}
          onClick={() => setTypeOverlayOpen(true)}
        />

        <div className={cn("grid gap-2", showRooms ? "grid-cols-2" : "grid-cols-1")}>
          {showRooms && (
            <FilterPickerField
          ...
          <div className="relative">
            <input
              ...
              placeholder="Цена до"
              value={priceMax}
              onChange={(e) => setPriceMax(e.target.value.replace(/[^\d\s]/g, ""))}
