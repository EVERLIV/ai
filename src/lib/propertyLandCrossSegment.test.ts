...
describe("land cross-segment visibility", () => {
  it("expands Участок ↔ Земля in filters", () => {
    expect(expandLandFilterTypes(["Участок"]).sort()).toEqual(["Участок", "Земля"].sort());
    expect(expandLandFilterTypes(["Земля"]).sort()).toEqual(["Участок", "Земля"].sort());
  });

  it("matches commercial Земля when filtering residential Участок", () => {
    const land = { segment: "commercial" as const, type: "Земля", extras: null };
  ...
  it("keeps apartments out of land filter", () => {
    const flat = { segment: "residential" as const, type: "Квартира", extras: null };
