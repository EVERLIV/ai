...
export function buildSeoDescription(form: PropertyFormState): string {
  ...
  const cta =
    deal === "Продажа"
      ? "Смотрите объявление на ДАДАТУТ и запишитесь на просмотр — ответим быстро."
      : deal === "Посуточно"
      ? "Свободные даты и бронь — на ДАДАТУТ."
      : "Условия и просмотр — на ДАДАТУТ, напишите в объявлении.";
