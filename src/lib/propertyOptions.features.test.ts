...
describe("getFeatureGroupsFor", () => {
  it("hides land/house plot group for apartments", () => {
    const titles = getFeatureGroupsFor("residential", ["Квартира"]).map(
      (g) => g.title,
    );
  ...
  it("includes house plot features for houses", () => {
    const titles = getFeatureGroupsFor("residential", ["Дом"]).map(
      (g) => g.title,
    );
  ...
  it("limits commercial land to land group", () => {
    const titles = getFeatureGroupsFor("commercial", ["Земля"]).map(
      (g) => g.title,
    );
  ...
  it("keeps office without warehouse/land groups", () => {
    const titles = getFeatureGroupsFor("commercial", ["Офис"]).map(
      (g) => g.title,
    );
