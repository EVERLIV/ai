  ...
  return (
    <>
      ...
      <ul>
        ...
        <li>
          направление информационных и рекламных писем — только при наличии
          отдельного согласия (opt-in). Отписаться можно по ссылке в письме или
          на странице{" "}
      ...
      <p>
        ...
        (хостинг, аналитика, рассылки, защита форм), в объёме, необходимом для
        оказания услуг. Для защиты от автоматизированных заявок используется
        сервис Яндекс SmartCaptcha (ООО «Яндекс.Облако»); при проверке могут
        обрабатываться технические данные браузера и сетевые сведения в
        соответствии с политикой Яндекса.
      </p>
      <h2>6. Права пользователя</h2>
      <p>
        Вы вправе запросить доступ, уточнение, блокирование или удаление своих
        персональных данных, направив обращение на{" "}
        <a href={`mailto:${CONTACTS.email}`}>{CONTACTS.email}</a>. Отказаться от
        рекламных рассылок можно в любой момент по ссылке «Отписаться» в письме
        или на странице <Link to="/unsubscribe">отписки</Link> — без входа в
        аккаунт.
      </p>
      <h2>7. Cookie и локальное хранилище</h2>
      <p>
        Для работы Сервиса используются технические данные в браузере: сессия
        авторизации, избранное, сравнение объектов и настройки интерфейса
        (чаще через localStorage). Служебные cookie могут применяться
        инфраструктурой сайта и виджетами (например, защита форм). Сторонние
        cookie рекламных счётчиков (Яндекс.Метрика, Google Analytics и т.п.) на
        сайте не подключаются. Продолжая пользоваться сайтом, вы соглашаетесь с
        использованием указанных технологий в объёме, необходимом для работы
        Сервиса.
      </p>
