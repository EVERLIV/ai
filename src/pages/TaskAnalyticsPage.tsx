...
const insightColors: Record<
  string,
  { bg: string; border: string; icon: React.ElementType; text: string }
> = {
  critical: {
    bg: "bg-red-50",
...
function ReportCard({
  ...
  return (
    <div
      className={`bg-white rounded-xl border transition-all ${isToday ? "border-indigo-200 shadow-md" : "border-gray-200"}`}
    >
      {/* Заголовок */}
      <button
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[10px] text-gray-400">
            {new Date(report.generated_at).toLocaleTimeString("ru-RU", {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </span>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <div className={`text-xs font-bold mb-1 ${style.text}`}>
                        {insight.title}
                      </div>
                      <p
                        className={`text-xs leading-relaxed ${style.text} opacity-80`}
                      >
                        {insight.text}
                      </p>
                    </div>
...
export default function TaskAnalyticsPage() {
  ...
  const _done = tasks.filter((t) => t.status === "done").length;
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-4 md:py-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Ежедневный дайджест формируется в 22:00
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              ...
              <div
                key={s.label}
                className={`${s.bg} rounded-xl border border-gray-200 p-4 text-center`}
              >
          ...
          {!todayReport && !isLoading && (
            <div className="bg-white border border-dashed border-indigo-300 rounded-xl p-8 text-center mb-6">
              ...
              <p className="text-sm font-semibold text-gray-700 mb-1">
                Отчёт за сегодня ещё не сформирован
              </p>
              <p className="text-xs text-gray-400 mb-4">
                Автоматически генерируется в 22:00 или нажмите кнопку выше
              </p>
              <button
            ...
            <div className="space-y-3">
              {reports.map((report) => (
                <ReportCard
                  key={report.id}
                  report={report}
                  isToday={report.report_date === today}
                />
              ))}
              {reports.length === 0 && (
