...
import { formatCountdown, getDeadlineInfo } from "@/lib/deadline";
...
export default function TaskDetailPage() {
  ...
  const authorName =
    user?.user_metadata?.full_name || user?.email?.split("@")[0] || "Аноним";
  ...
  const addCheckItem = () => {
    ...
    const updated = [
      ...checklist,
      { id: crypto.randomUUID(), text: newCheckItem.trim(), done: false },
    ];
    updateTask.mutate({ id: task.id, checklist: updated });
    setNewCheckItem("");
  };
  ...
  const removeTag = (tag: string) =>
    updateTask.mutate({ id: task.id, tags: tags.filter((t) => t !== tag) });
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6 py-4 md:py-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 mb-4">
            <div className="flex items-start gap-3 mb-4">
              <span
                className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${statusColors[task.status]}`}
              >
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-3">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="group inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full"
                >
                  {tag}
                  <button
          ...
          {deadline && task.status !== "done" && (
            <div
              className={`rounded-xl border p-4 mb-4 flex items-center gap-3 md:gap-4 ${
              ...
              <div className="mb-3">
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-3">
                  <div
                    ...
                    style={{
                      width: `${checklist.length ? (checkDone / checklist.length) * 100 : 0}%`,
                    }}
                  />
                </div>
                <div className="space-y-1.5">
                  {checklist.map((item) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm flex-1 ${item.done ? "line-through text-gray-400" : "text-gray-700"}`}
                      >
                        {item.text}
                      </span>
                      <button
