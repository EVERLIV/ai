...
export default function SuperAdmin() {
  ...
  const resetPasswordMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Новый пароль установлен успешно",
      });
  ...
  const stats = {
    ...
    clients: users.filter(
      (u) => !u.roles.includes("admin") && !u.roles.includes("manager"),
    ).length,
  ...
  return (
    <div className="min-h-screen bg-background">
                        ...
                        <TableCell>
                          <div className="flex flex-wrap gap-1">
                            {u.roles.map((role) => (
                              <span
                                key={role}
                                className={`text-xs px-2 py-0.5 border rounded-full font-medium ${ROLE_COLORS[role]}`}
                              >
                                {ROLE_LABELS[role]}
                              </span>
                            ))}
                        ...
                        <TableCell className="text-right pr-6">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-52">
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  sendResetEmailMutation.mutate(u.email)
                                }
                              >
                              ...
                              {!u.confirmed && (
                                <DropdownMenuItem
                                  onClick={() =>
                                    confirmEmailMutation.mutate(u.id)
                                  }
                                >
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() => {
                                  if (
                                    confirm(`Удалить пользователя ${u.email}?`)
                                  ) {
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          !open && setRoleDialog({ open: false, user: null })
        }
      >
        ...
        <DialogContent className="sm:max-w-sm" aria-describedby={undefined}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                resetPasswordMutation.isPending || newPasswordValue.length < 6
              }
            >
              <Key className="w-4 h-4 mr-2" />
              Сохранить
