...
import { CONTACTS, COMPANY } from "@/config/company";
...
const perks = [
  { icon: Users, text: "Команда без бюрократии — один менеджер ведёт сделку до конца" },
  { icon: Heart, text: "Уважение к людям и прозрачные условия работы" },
  { icon: Sparkles, text: "Реальные объекты и сделки, не «виртуальный» каталог" },
];

export default function VacanciesPage() {
  const [scrollPct, setScrollPct] = useState(0);
  const [selectedVacancy, setSelectedVacancy] = useState(VACANCIES[0]?.title ?? "");
  ...
  const handleApplyClick = (title: string) => {
    ...
    setSent(false);
    setError(null);
    document.getElementById("vacancy-apply")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const resumeNote = message.trim() ? `Резюме / комментарий: ${message.trim()}` : null;
      await submitLead({
        name: trimmedName,
        phone: trimmedPhone,
      ...
      setError("Не удалось отправить отклик. Позвоните нам или напишите на почту.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="relative h-[280px] lg:h-[320px]">
            ...
            <div className="absolute inset-0 flex items-center">
              <div className="container mx-auto px-4 lg:px-8">
                <p className="text-primary text-xs font-semibold uppercase tracking-widest mb-3">Карьера</p>
                ...
                <p className="text-background/75 text-base max-w-2xl leading-relaxed">
                  ...
                  Открыты позиции менеджера по аренде и юриста — заработная плата обсуждается на собеседовании.
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex-1 min-w-0 space-y-8">
                <div className="grid sm:grid-cols-3 gap-3">
                  {perks.map((p) => {
                    const Icon = p.icon;
                    return (
                      <div key={p.text} className="flex gap-3 p-4 bg-muted/30 border border-border">
                ...
                <div className="space-y-6">
                  <div>
                    <h2 className="font-display text-2xl font-bold text-foreground mb-1">Открытые позиции</h2>
                    <p className="text-sm text-muted-foreground">
                      {VACANCIES.length} вакансии · отклик через форму ниже или по телефону
                    </p>
                  </div>
                  {VACANCIES.map((vacancy) => (
                    <VacancyCard key={vacancy.id} vacancy={vacancy} onApply={handleApplyClick} />
                  ))}
                </div>

                <div id="vacancy-apply" className="bg-card border border-border p-6 sm:p-8 scroll-mt-28">
                  {sent ? (
                    <div className="text-center py-6 space-y-3">
                      <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto" />
                      <h2 className="font-display text-xl font-bold text-foreground">Отклик отправлен</h2>
                      <p className="text-sm text-muted-foreground max-w-md mx-auto">
                        Мы свяжемся с вами в рабочее время. Можно также позвонить: {CONTACTS.phone}
                      </p>
                      <button
                        type="button"
                    ...
                    <>
                      <h2 className="font-display text-xl font-bold text-foreground mb-1">Отклик на вакансию</h2>
                      <p className="text-sm text-muted-foreground mb-6">
                        Оставьте контакты и коротко расскажите о себе — мы перезвоним или напишем.
                      </p>
                      <form onSubmit={handleSubmit} className="space-y-4 max-w-xl">
                        <div>
                          <label className="text-[11px] font-medium text-muted-foreground mb-1.5 block">Вакансия</label>
                          <select
