...
export default function NewsPostPage() {
  ...
  const allPosts = useMemo(
    () => mergeNewsPosts(dbAllPosts ?? [], FALLBACK_NEWS_POSTS),
    [dbAllPosts],
  );
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <article className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mb-4 text-sm text-muted-foreground">
                  <span
                    className={`inline-block text-[11px] font-medium px-2 py-0.5 ${catColor}`}
                  >
                    {post.category}
                  </span>
                ...
                <h1 className="font-display text-3xl font-bold leading-snug mb-4">
                  {post.title}
                </h1>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {related.map((p) => (
                        <Link
                          key={p.id}
                          to={`/news/${p.slug}`}
                          className="group block border border-border bg-card hover:-translate-y-0.5 transition-all p-4"
                        >
                          <span
                            className={`inline-block text-[11px] font-medium px-2 py-0.5 mb-2 ${CATEGORY_COLORS[p.category] ?? "bg-muted text-muted-foreground"}`}
                          >
                            {p.category}
                          </span>
